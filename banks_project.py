...
def extract(url, table_attribs):
    ...
    for row in rows:
        if count < 50:
            col = row.find_all('td')
            if len(col) != 0:
                data_dict = {'Rank': col[0].contents[0][:-1],
            ...
            break
    # Print the table with banks in USD billion
    print("Table with Banks in USD Billion:")
    print(df[['Name', 'MC_USD_Billion']])
...
def transform(df, csv_path):
    ...
    print("\nTransformed Data within transform function:")
    print(df)
